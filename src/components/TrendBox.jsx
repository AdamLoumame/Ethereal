...
export default function TrendBox({ trend, format, large = false, largest = false, customSize = "" }) {
	...
	return (
		<Link to={`/${format || trend.media_type}/${trend.id}`} className={`z-10 flex flex-col group gap-2 ${customSize ? customSize : largest ? "min-w-1/5 max-w-1/5 max-lg:max-w-1/4 max-lg:min-w-1/4 max-sm:max-w-1/3 max-sm:min-w-1/3 p-2" : large ? "min-w-1/6 max-w-1/6 max-lg:max-w-1/5 max-lg:min-w-1/5 max-sm:max-w-1/4 max-sm:min-w-1/4 max-xs:max-w-2/5 max-xs:min-w-2/5 max-2xs:max-w-1/2 max-2xs:min-w-1/2" : "min-w-1/8 max-w-1/8 max-lg:max-w-1/5 max-lg:min-w-1/5 max-sm:max-w-1/3 max-sm:min-w-1/3"} cursor-pointer snap-start ${largest && "text-center text-xl"}`}>
